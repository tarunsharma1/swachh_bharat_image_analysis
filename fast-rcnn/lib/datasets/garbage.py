...
class garbage(datasets.imdb):
 ...
 #        print 'wrote ss roidb to {}'.format(cache_file)
	...
	#raw_data = sio.loadmat(filename)['all_boxes'].ravel()
        raw_data = sio.loadmat(filename)['all_boxes']
	# this next line was added by  me for the inria_25
	raw_data = raw_data[0]
	...
	objs = re.findall('\(\d+, \d+\)[\s\-]+\(\d+, \d+\)', data)
	    ...
	    coor = re.findall('\d+', obj)
            ...
            x2 = float(coor[2])
            y2 = float(coor[3])
            cls = self._class_to_ind['garbage']
